...
static void zns_dev_init(void *arg, std::string ip, std::string port)
{
    ...
    log_debug("static_cast worked");
    int rc = 0;
    ctx->m1.ctrlr = nullptr;
    log_debug("access ctrlr ");
    // ctx->m1->ns = NULL;
    ...
    ctx->m1.ctrlr = spdk_nvme_connect(&trid, &opts, sizeof(opts));
    log_debug("connect work");
    // ctx->ctrlr = spdk_nvme_connect(&trid, NULL, 0);

    if (ctx->m1.ctrlr == NULL && ctx->verbose) {
        fprintf(stderr,
    ...
    log_debug("21");
    if (spdk_nvme_zns_ctrlr_get_data(ctx->m1.ctrlr) && ctx->verbose) {
        printf("ZNS Specific Controller Data\n");
        printf("============================\n");
        printf("Zone Append Size Limit:      %u\n",
               spdk_nvme_zns_ctrlr_get_data(ctx->m1.ctrlr)->zasl);
        printf("\n");
        printf("\n");
...
static void zstore_qpair_teardown(void *arg)
{
    ...
    if (ctx->verbose)
        log_info("complete queued items {}, qd {}", ctx->m1.num_queued,
                 ctx->qd);
...
int z_reset(void *arg, uint64_t slba, bool all)
{
    ...
    int rc = spdk_nvme_zns_reset_zone(
        ctx->m1.ns, ctx->m1.qpair, slba, /* starting LBA of the zone to reset */
...
int z_get_device_info(void *arg)
{
    ...
    ERROR_ON_NULL(ctx->m1.ctrlr, 1);
    ERROR_ON_NULL(ctx->m1.ns, 1);

    u64 zone_dist = 0x80000; // zone size
    ctx->zslba = zone_dist * ctx->current_zone;

    const struct spdk_nvme_ns_data *ns_data = spdk_nvme_ns_get_data(ctx->m1.ns);
    const struct spdk_nvme_zns_ns_data *ns_data_zns =
        spdk_nvme_zns_ns_get_data(ctx->m1.ns);
    const struct spdk_nvme_ctrlr_data *ctrlr_data =
        spdk_nvme_ctrlr_get_data(ctx->m1.ctrlr);
    const spdk_nvme_zns_ctrlr_data *ctrlr_data_zns =
        spdk_nvme_zns_ctrlr_get_data(ctx->m1.ctrlr);
    union spdk_nvme_cap_register cap =
        spdk_nvme_ctrlr_get_regs_cap(ctx->m1.ctrlr);
...
static void __complete(void *arg, const struct spdk_nvme_cpl *cpl)
{
    ...
    ctx->m1.num_completed += 1;
    if (spdk_nvme_cpl_is_error(cpl)) {
        spdk_nvme_qpair_print_completion(ctx->m1.qpair,
        ...
        if (ctx->verbose)
            log_debug("operation complete: queued {} completed {} success {} "
                      "fail {}",
                      ctx->m1.num_queued, ctx->m1.num_completed,
                      ctx->m1.num_success, ctx->m1.num_fail);
        // if (ctx->verbose)
...
int z_read(void *arg, uint64_t slba, void *buffer, uint64_t size)
{
    ...
    while (lbas_processed < lbas) {
        ...
        if (ctx->verbose)
            log_info("cmd_read: slba_start {}, current step size {}, queued {}",
                     slba_start, current_step_size, ctx->m1.num_queued);
        rc = spdk_nvme_ns_cmd_read(ctx->m1.ns, ctx->m1.qpair,
                                   (char *)buffer +
    ...
    while (ctx->m1.num_queued) {
        if (ctx->verbose)
            log_info("qpair process completion: queued {}, qd {}",
                     ctx->m1.num_queued, ctx->qd);
    ...
    while (lbas_processed < lbas) {
        ...
        if (ctx->verbose)
            log_info(
                ...
                slba_start, current_step_size, ctx->m1.num_queued);
        rc = spdk_nvme_zns_zone_append(ctx->m1.ns, ctx->m1.qpair,
                                       (char *)buffer +
                                           lbas_processed * ctx->info.lba_size,
                                       slba_start,        /* LBA start */
                                       current_step_size, /* number of LBAs */
...
int z_get_zone_head(void *arg, uint64_t slba, uint64_t *head)
{
    ...
    size_t report_bufsize = spdk_nvme_ns_get_max_io_xfer_size(ctx->m1.ns);
    uint8_t *report_buf = (uint8_t *)calloc(1, report_bufsize);
    Completion completion = {.done = false, .err = 0};
    int rc = spdk_nvme_zns_report_zones(
        ctx->m1.ns, ctx->m1.qpair, report_buf, report_bufsize, slba,
