...
{
  public:
    Segment(ZstoreController *zstoreController, uint32_t segmentId,
            RequestContextPool *ctxPool, ReadContextPool *rctxPool,
            StripeWriteContextPool *sctxPool, uint32_t configurationId = 0);
    Segment(ZstoreController *zstoreController, RequestContextPool *ctxPool,
            ReadContextPool *rctxPool, StripeWriteContextPool *sctxPool,
            SegmentMetadata *segMeta);
    void initSegment(ZstoreController *zstoreController,
                     RequestContextPool *ctxPool, ReadContextPool *rctxPool,
     ...
     * RAIZN_SIMPLE.
     *
     * @return >0 the size successfully issued the block to the drive (excluding
     * the metadata)
     */
     ...
     * @return 0 the segment is busy; cannot proceed to issue
     */
    uint32_t BufferedAppend(RequestContext *ctx, uint32_t offset,
                            uint32_t size);
     ...
     * @return false the segment is busy (no read context remains); cannot
     * proceed to read
     */
     ...
     *
     */
    bool ReadValid(RequestContext *ctx, uint32_t offset, PhysicalAddr phyAddr,
                   uint32_t size);
     ...
     *        Called by controller after the data region is full and the footer
     * region is persisted
     */
    void Seal();
     ...
     *
     * @return true The segment is full and cannot accept new blocks; need to
     * write the footer
    ...
    void GenerateParityBlock(StripeWriteContext *stripe, uint32_t zonePos,
                             uint32_t stripeId, struct timeval,
                             bool useMetaZone);
    ...
    void
    RecoverIndexFromSealedSegment(uint8_t *buffer,
                                  std::pair<uint64_t, PhysicalAddr> *indexMap);
    void
    RecoverIndexFromOpenSegment(std::pair<uint64_t, PhysicalAddr> *indexMap);
  ...
  private:
    ...
    void encodeStripe(uint8_t **stripe, uint32_t n, uint32_t k,
                      uint32_t unitSize);
    void decodeStripe(uint32_t offset, uint8_t **stripe, bool *alive,
                      uint32_t n, uint32_t k, uint32_t decodeIndex);
    ...
    ZstoreController *mZstoreController;
