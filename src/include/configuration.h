...
class Conf
{
  public:
    ...
    // All the following getters read from fixed values
    ...
    static int GetNumOfGw() { return GetInstance().gNumofGw; }
    static int GetRdmaBufferSize() { return GetInstance().gRdmaBufferSize; }
    ...
    static bool IsReplay() { return GetInstance().gReplayWorkload; }
    static bool IsExperimental() { return GetInstance().gExperimental; }
    static bool IsDebugging() { return GetInstance().gDebug; }
    ...
    static int GetQpairSize() { return GetInstance().gQpairSize; }
  ...
  private:
    ...
    uint64_t gZoneDist = 524288; // zone size: 0x80000
    uint64_t gZoneCap = 275456;  // zone capacity: 0x43500
    ...
    int gBlockSize = 4096;
    int gRdmaBufferSize = 128;

    // Configured parameters: 64/256/4096
    int gContextPoolSize = 4096 * 10;
    ...
    bool gExperimental = false; // this will turn on all logs
};
