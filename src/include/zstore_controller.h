...
class ZstoreController
{
  public:
    ...
    Result<std::vector<ObjectKeyHash>> ListObjects(const int max_keys);
    ...
    // Inflight write APIs
    Result<bool> SearchInflight(const ObjectKeyHash &key_hash);
    Result<bool> BroadcastToInflight(const ObjectKeyHash &key_hash);
    Result<bool> BroadcastToInflight(const ObjectKeyHash &key_hash,
                                     const RequestContext *s1,
                                     const RequestContext *s2,
                                     const RequestContext *s3);

    BroadcastSet mBroadcast;
    // Broadcast APIs
    Result<bool> AddToBroadcast(const ObjectKeyHash &key_hash);
    Result<bool> RemoveFromBroadcast(const ObjectKeyHash &key_hash);

    // ZStore condvar map: this is used to signal the completion (phase 2) of a
    // request
    CondMap mCondMap;
    void WaitForPhase2(ObjectKeyHash key);
    void ReleaseKey(ObjectKeyHash key);
    ...
    std::unordered_map<uint32_t, RequestContextPool *>
        mReplayRequestContextPool;
    ...
    Device *GetDeviceById(const unsigned long &index)
    {
    ...
    void Map2Gc(const ZstoreMap &hashmap, std::vector<char *> &tombstoneObjects,
    ...
    // struct BufferEntry mRdmaSrcBuffer;
    // struct BufferEntry mRdmaDstBuffer;
    // struct RdmaServer mRdmaServer;
    // struct sockaddr_in mRdmaServerSock;
    // struct sockaddr_in mRdmaClientSock;

    struct RdmaClient mRdmaClient;
    std::jthread mRdmaServerThread;
    // u32 mRdmaEntrySize = 128;                // 2^26 in bytes, which is less
    // u32 mRdmaBufferSize = 128 * 1024 * 1024; // 2^26 in bytes, which is less
    // than 2^32
  ...
  private:
    ...
    // 500k, 1 million, 2 million, 4 million, 10 million
    // int mCkptMapSize = 500'000;
    // int mCkptMapSize = 1'000'000;
    // int mCkptMapSize = 2'000'000;
    // int mCkptMapSize = 5'000'000;
    // int mCkptMapSize = 10'000'000;
    // int mCkptRecentMapSize = mCkptMapSize * 0.01;
    // GC with half zone
    int mCkptMapSize = 64'000 * 6;
    int mCkptRecentMapSize = mCkptMapSize;

    bool mCkpt = false;
    bool mHasRunCkpt = false;
    ...
    std::vector<int> mObjectSizeInBlocks = {
        1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192};

    std::unordered_map<int, int> mObjectSizePoolMap = {
        {1, 4096},    {2, 4096},    {4, 4096},    {8, 4096},   {16, 4096},
        {32, 4096},   {64, 4096},   {128, 4096},  {256, 4096}, {512, 4096},
        {1024, 4096}, {2048, 4096}, {4096, 4096}, {8192, 4096}};
};

// kym::endpoint::Options RdmaOpts = {
//     .qp_attr =
//         {
//             .cap =
//                 {
