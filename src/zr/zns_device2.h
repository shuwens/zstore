...
// const int append_times = 128000 * 2;
...
static void zns_dev_init(void *arg)
{
    ...
    snprintf(trid.traddr, sizeof(trid.traddr), "%s", "12.12.12.2");
    snprintf(trid.trsvcid, sizeof(trid.trsvcid), "%s", "5520");
...
static void zstore_qpair_teardown(void *arg)
{
    ...
    if (ctx->verbose)
        log_info("complete queued items {}, qd {}", ctx->num_queued, ctx->qd);
