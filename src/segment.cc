...
Segment::Segment(ZstoreController *zstoreController, uint32_t segmentId,
                 ...
                 StripeWriteContextPool *sctxPool)
{
    ...
    mZstoreController = zstoreController;
    mPos = 0;
    mPosInStripe = 0;
    ...
    mAdminStripe->data = new uint8_t
        *[Configuration::GetStripeSize() / Configuration::GetBlockSize()];
    mAdminStripe->metadata = new uint8_t
        *[Configuration::GetStripeSize() / Configuration::GetBlockSize()];
    ...
    mHeaderRegionSize = zstoreController->GetHeaderRegionSize();
    mDataRegionSize = zstoreController->GetDataRegionSize();
    mFooterRegionSize = zstoreController->GetFooterRegionSize();
    ...
    mCodedBlockMetadata =
        new CodedBlockMetadata[mSegmentMeta.n * mDataRegionSize];
...
const std::vector<Zone *> &Segment::GetZones() { return mZones; }

uint32_t Segment::GetNumBlocks() const { return mNumBlocks; }

uint32_t Segment::GetNumInvalidBlocks() const { return mNumInvalidBlocks; }

bool Segment::IsFull() { return mPos == mHeaderRegionSize + mDataRegionSize; }
...
void Segment::recycleContexts() { recycleStripeWriteContexts(); }
...
SegmentStatus Segment::GetStatus() { return mSegmentStatus; }
...
void progressFooterWriter(void *args) { progressFooterWriter2(args, nullptr); }
...
bool Segment::StateTransition()
{
    ...
    if (mSegmentStatus == SEGMENT_NORMAL) {
        if (mPos == mHeaderRegionSize + mDataRegionSize) {
            ...
            if (!Configuration::InjectCrash() &&
                timestamp - mLastStripeCreationTimestamp >= 0.001) {
                // fill and flush the last stripe if no further write request
                // for 200us interval
                FlushCurrentStripe();
            }
        }
    ...
    if (mSegmentStatus == SEGMENT_CONCLUDING_APPENDS_IN_GROUP ||
        ...
        if (checkStripeAvailable(mCurStripe) &&
            mStripeWriteContextPool->NoInflightStripes()) {
        ...
        if (checkStripeAvailable(mCurStripe)) {
            if (CanSeal()) {
                ...
                thread_send_msg(mZstoreController->GetHttpThread(),
                                progressFooterWriter, this);
                // } else {
                //     event_call(Configuration::GetEcThreadCoreId(),
                //                progressFooterWriter2, this, nullptr);
                // }
            }
        }
...
void Segment::FinalizeCreation()
{
    ...
    // if (!Configuration::GetEventFrameworkEnabled()) {
    thread_send_msg(mZstoreController->GetHttpThread(), finalizeSegmentHeader,
                    this);
    // } else {
    //     event_call(Configuration::GetEcThreadCoreId(),
    //     finalizeSegmentHeader2,
    //                this, nullptr);
    // }
}
...
void Segment::ProgressFooterWriter()
{
    ...
    uint32_t begin = (mPos - mHeaderRegionSize - mDataRegionSize) *
                     (Configuration::GetBlockSize() / (8 + 8 + 4));
    uint32_t end = std::min(
        mDataRegionSize, begin + Configuration::GetBlockSize() / (8 + 8 + 4));
    for (uint32_t i = 0; i < mSegmentMeta.n; ++i) {
        uint32_t base = i * mDataRegionSize;
        uint32_t pos = 0;
        for (uint32_t offset = begin; offset < end; ++offset) {
            uint8_t *footerForCurrentBlock =
                mCurStripe->ioContext[i]->dataBuffer + (offset - begin) * 20;
            *(uint64_t *)(footerForCurrentBlock + 0) =
                mCodedBlockMetadata[base + offset].lba;
            *(uint64_t *)(footerForCurrentBlock + 8) =
                mCodedBlockMetadata[base + offset].timestamp;
...
bool Segment::findStripe()
{
    ...
    if (mSegmentStatus == SEGMENT_NORMAL) {
        ...
        if (stripe == nullptr) {
            ...
            for (int j = 0; j < Configuration::GetStripeParitySize() /
...
{
    struct GenerateParityBlockArgs *gen_args =
        reinterpret_cast<struct GenerateParityBlockArgs *>(arg1);
    Segment *segment = reinterpret_cast<Segment *>(gen_args->segment);
    StripeWriteContext *stripe =
        reinterpret_cast<StripeWriteContext *>(gen_args->stripe);
    uint32_t zonePos = gen_args->zonePos;
    free(gen_args);
    segment->GenerateParityBlock(stripe, zonePos);
}

void generateParityBlock(void *args) { generateParityBlock2(args, nullptr); }
...
bool Segment::Append(RequestContext *ctx, uint32_t offset)
{
    ...
    uint32_t zoneId =
        Configuration::CalculateDiskId(mPos, whichBlock, mSegmentMeta.numZones);
    // Issue data block
    {
        RequestContext *slot = mRequestContextPool->GetRequestContext(true);
...
void Segment::GenerateParityBlock(StripeWriteContext *stripe, uint32_t zonePos)
{
    ...
    //         memcpy(stripe->ioContext[i]->data, stripe->ioContext[0]->data,
    //                Configuration::GetBlockSize());
    //     }
    // } else if (Configuration::GetRaidLevel() == RAID01) {
    //     for (uint32_t i = k; i < n; ++i) {
    //         memcpy(stripe->ioContext[i]->data, stripe->ioContext[i -
    //         k]->data,
    //                Configuration::GetBlockSize());
    //     }
    ...
    //             &(((BlockMetadata
    //             *)stripe->ioContext[i]->meta)->fields.coded));
    //     }
    //     EncodeStripe(stripeData, n, k, Configuration::GetBlockSize());
    //     EncodeStripe(stripeProtectedMetadata, n, k,
    //                  16); // 16 bytes for the protected fields
    //                       //    uint32_t zoneId = 0;
    //                       //    for (uint32_t i = 0; i < n; ++i) {
    //     //      uint32_t diskId = Configuration::CalculateDiskId(
    //     //          zonePos, i, (RAIDLevel)mSegmentMeta.raidScheme,
    //     //          mSegmentMeta.numZones);
    //     //      if (diskId == 0) { zoneId = i; }
    //     ////      printf("%u %u %u %lu %lu %u\n", i, diskId,
    //     ///((BlockMetadata*)stripe->ioContext[i]->meta)->fields.coded.lba,
    //     ///((BlockMetadata*)stripe->ioContext[i]->meta)->fields.coded.timestamp,
    //     ///((BlockMetadata*)stripe->ioContext[i]->meta)->fields.replicated.stripeId);
    //     //    }
    //     //    printf("%u %lu %lu %u\n", zoneId,
    //     //
    //     ((BlockMetadata*)stripe->ioContext[zoneId]->meta)->fields.coded.lba,
    //     //
    //     ((BlockMetadata*)stripe->ioContext[zoneId]->meta)->fields.coded.timestamp,
    //     //
    //     ((BlockMetadata*)stripe->ioContext[zoneId]->meta)->fields.replicated.stripeId);
    // }

    for (uint32_t i = k; i < n; ++i) {
        uint32_t zoneId =
            Configuration::CalculateDiskId(zonePos, i, mSegmentMeta.numZones);
        RequestContext *slot = stripe->ioContext[i];
        slot->lba = ~0ull;
        slot->ctrl = mZstoreController;
        slot->segment = this;
        slot->zoneId = zoneId;
        slot->stripeId =
            (zonePos - mHeaderRegionSize) % Configuration::GetStripeGroupSize();
        slot->status = WRITE_REAPING;
        slot->type = STRIPE_UNIT;
        // if (mode == ZONEWRITE_ONLY ||
        //     (mode == ZAPRAID &&
        //      zonePos % Configuration::GetStripeGroupSize() == 0)) {
        //     slot->append = false;
        // } else {
        slot->append = true;
        // }
...
bool Segment::ReadValid(RequestContext *ctx, uint32_t pos, PhysicalAddr phyAddr,
                        bool *isValid)
{
    bool success = false;
    ...
    if (offset < mHeaderRegionSize ||
        offset >= mHeaderRegionSize + mDataRegionSize) {
        *isValid = false;
        success = true;
    } else if (mValidBits[zoneId * mDataRegionSize + offset -
                          mHeaderRegionSize] == 0) {
        *isValid = false;
        success = true;
    } else {
...
void Segment::Reset(RequestContext *ctx)
{
    ...
    for (int i = 0; i < mSegmentMeta.n; ++i) {
        ...
        context->ctrl = mZstoreController;
        context->segment = this;
...
void Segment::ReadStripe(RequestContext *ctx)
{
    ...
    uint32_t n =
        Configuration::GetStripeSize() / Configuration::GetStripeUnitSize();
    uint32_t k =
        Configuration::GetStripeDataSize() / Configuration::GetStripeUnitSize();
    ...
    //             } else {
    //                 uint8_t *tmp = mCompactStripeTable + index * 3;
    //                 stripeIdOfCurBlock = (*(tmp + 2) << 16) | *(uint16_t
    //                 *)tmp;
    //             }
    //
    ...
    //                            index, stripeIdOfCurBlock, stripeId);
    //                     //            uint8_t *tmp = mCompactStripeTable +
    //                     index
    //                     //            * 3; printf("%p %p %p %lu %lu\n", this,
    //                     //            tmp, mCompactStripeTable, index, *(tmp
    //                     +
    //                     //            2), *(uint16_t*)tmp);
    //                 }
    ...
    for (uint32_t i = 0; i < n && cnt < k; ++i) {
        ...
        mZones[i]->Read(realOffsets[i], Configuration::GetStripeUnitSize(),
                        reqCtx);
...
void Segment::WriteComplete(RequestContext *ctx)
{
    ...
    // Note that here the (lba, timestamp, stripeId) is not the one written to
    // flash page Thus the lba and timestamp is "INVALID" for parity block, and
...
uint32_t Segment::GetSegmentId() { return mSegmentMeta.segmentId; }

uint64_t Segment::GetCapacity() const { return mDataRegionSize; }
...
void Segment::RecoverLoadAllBlocks()
{
    ...
    for (uint32_t i = 0; i < numZones; ++i) {
        mDataBufferForRecovery[i] =
            (uint8_t *)spdk_zmalloc(mDataRegionSize * blockSize, 4096, NULL,
                                    SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
        mMetadataBufferForRecovery[i] =
            (uint8_t *)spdk_zmalloc(mDataRegionSize * metadataSize, 4096, NULL,
                                    SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
    }
        ...
        for (uint32_t i = 0; i < numZones; ++i) {
            ...
            if (numBlocks2Fetch > 0) {
                ...
                if ((error = spdk_nvme_ns_cmd_read_with_md(
                         ...
                         mDataBufferForRecovery[i] +
                             offsetInDataRegion * blockSize,
                         mMetadataBufferForRecovery[i] +
                             offsetInDataRegion * metadataSize,
                         zslba + offsets[i], numBlocks2Fetch, completeOneEvent,
                         &counter, 0, 0, 0)) < 0) {
        ...
        while (counter != 0) {
            for (uint32_t i = 0; i < numZones; ++i) {
                spdk_nvme_qpair_process_completions(
                    mZones[i]->GetDevice()->GetIoQueue(0), 0);
            }
        }
    }
    ...
    for (uint32_t i = 0; i < numZones; ++i) {
        ...
        for (uint32_t j = 0; j < wp - zslba - mHeaderRegionSize; ++j) {
            ...
            BlockMetadata *blockMeta =
                &(((BlockMetadata *)mMetadataBufferForRecovery[i])[j]);
            ...
            uint32_t groupSize = Configuration::GetStripeGroupSize();
            stripeOffset =
                mHeaderRegionSize + j / groupSize * groupSize + stripeId;
            // }

            uint32_t whichBlock = 0;
            if (i == Configuration::CalculateDiskId(
                         stripeOffset, mZones.size() - 1, mZones.size())) {
                // parity
                ...
                mCodedBlockMetadata[index].timestamp =
                    blockMeta->fields.coded.timestamp;
            }
...
bool Segment::RecoverFooterRegionIfNeeded()
{
    ...
    uint8_t *footerBlock = (uint8_t *)spdk_zmalloc(
        Configuration::GetBlockSize(), Configuration::GetBlockSize(), NULL,
        SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
    uint32_t offsets[numZones];
    for (uint32_t i = 0; i < numZones; ++i) {
    ...
    for (uint32_t i = 0; i < numZones; ++i) {
        ...
        for (uint32_t pos = offsets[i];
             pos < mHeaderRegionSize + mDataRegionSize + mFooterRegionSize;
             ++pos) {
            uint32_t begin = (pos - 1 - mHeaderRegionSize - mDataRegionSize) *
                             (blockSize / 20);
            ...
            bool done = false;
            for (uint32_t offset = begin; offset < end; ++offset) {
                *(uint64_t *)(footerBlock + (offset - begin) * 20 + 0) =
                    mCodedBlockMetadata[base + offset].lba;
                *(uint64_t *)(footerBlock + (offset - begin) * 20 + 8) =
                    mCodedBlockMetadata[base + offset].timestamp;
    ...
    uint8_t *buffer, std::pair<uint64_t, PhysicalAddr> *indexMap)
{
    ...
    for (uint32_t zid = 0; zid < numZones; ++zid) {
        ...
        for (uint32_t offsetInFooterRegion = 0;
             offsetInFooterRegion < mFooterRegionSize; ++offsetInFooterRegion) {
            uint32_t begin = offsetInFooterRegion * (blockSize / 20);
            uint32_t end = std::min(mDataRegionSize, begin + blockSize / 20);

            for (uint32_t offsetInDataRegion = begin; offsetInDataRegion < end;
                 ++offsetInDataRegion) {
                uint8_t *metadataForCurrentBlock =
                    buffer + offsetInFooterRegion * blockSize +
                    (offsetInDataRegion - begin) * 20;
                uint64_t lba = *(uint64_t *)(metadataForCurrentBlock + 0);
                uint64_t writeTimestamp =
                    *(uint64_t *)(metadataForCurrentBlock + 8);
                uint32_t stripeId = *(uint32_t *)(metadataForCurrentBlock + 16);

                // if (Configuration::GetSystemMode() == ZAPRAID) {
                //     // update compact stripe table first
                //     uint32_t index = zid * mDataRegionSize +
                //     offsetInDataRegion; if
                ...
                if (indexMap[lba / Configuration::GetBlockSize()]
                            .second.segment == nullptr ||
                    writeTimestamp >
                        indexMap[lba / Configuration::GetBlockSize()].first) {
                    indexMap[lba / Configuration::GetBlockSize()] =
                        std::make_pair(writeTimestamp, pba);
                }
            }
        }
    }
    ...
    std::pair<uint64_t, PhysicalAddr> *indexMap)
{
    ...
    for (uint32_t zid = 0; zid < mZones.size(); ++zid) {
        for (uint32_t offset = 0; offset < mPos; ++offset) {
            ...
            if (indexMap[lba / Configuration::GetBlockSize()].second.segment ==
                    nullptr ||
                writeTimestamp >
                    indexMap[lba / Configuration::GetBlockSize()].first) {
                indexMap[lba / Configuration::GetBlockSize()] =
...
bool Segment::RecoverNeedRewrite()
{
    ...
    //                 stripeId = (*(tmp + 2) << 16) | *(uint16_t *)tmp;
    //             }
    //             if (mStripesToRecover.find(stripeId) ==
    //                 mStripesToRecover.end()) {
...
void Segment::RecoverFromOldSegment(Segment *oldSegment)
{
    ...
    for (uint32_t i = 0; i < mZones.size(); ++i) {
        ...
        if (spdk_nvme_ns_cmd_write_with_md(
                zone->GetDevice()->GetNamespace(),
                zone->GetDevice()->GetIoQueue(0), contexts[i]->dataBuffer,
                contexts[i]->metadataBuffer, zone->GetSlba(), 1,
                completeOneEvent, &counter, 0, 0, 0) != 0) {
            fprintf(stderr,
                    "Write error in writing the header for new segment.\n");
        }
    }
    ...
    for (uint32_t i = 0; i < checkpoint; ++i) {
        ...
        for (uint32_t j = 0; j < mZones.size(); ++j) {
            Zone *zone = mZones[j];
            memcpy(contexts[j]->dataBuffer,
                   oldSegment->mDataBufferForRecovery[j] +
                       i * Configuration::GetBlockSize(),
                   4096);
            memcpy(contexts[j]->metadataBuffer,
                   oldSegment->mMetadataBufferForRecovery[j] +
                       i * Configuration::GetMetadataSize(),
                   4096);
            if (spdk_nvme_ns_cmd_write_with_md(
                    zone->GetDevice()->GetNamespace(),
                    zone->GetDevice()->GetIoQueue(0), contexts[j]->dataBuffer,
                    contexts[j]->metadataBuffer,
                    zone->GetSlba() + mHeaderRegionSize + i, 1,
                    completeOneEvent, &counter, 0, 0, 0) != 0) {
                fprintf(stderr, "Write error in writing the previous groups "
                                "for new segment.\n");
            }
            uint32_t index = j * mDataRegionSize + i;
            mCodedBlockMetadata[index].lba =
                oldSegment->mCodedBlockMetadata[index].lba;
            mCodedBlockMetadata[index].timestamp =
                oldSegment->mCodedBlockMetadata[index].timestamp;
            if (Configuration::GetStripeGroupSize() <= 256) {
                mCompactStripeTable[index] =
                    oldSegment->mCompactStripeTable[index];
            } else if (Configuration::GetStripeGroupSize() <= 65536) {
                ((uint16_t *)mCompactStripeTable)[index] =
                    oldSegment->mCompactStripeTable[index];
            } else {
                uint8_t *tmp = mCompactStripeTable + index * 3;
                uint8_t *oldStripeId =
                    oldSegment->mCompactStripeTable + index * 3;
                *(tmp + 2) = *(oldStripeId + 2);
    ...
    for (auto stripe : oldSegment->mStripesToRecover) {
        ...
        for (uint32_t i = 0; i < numZones; ++i) {
            ...
            BlockMetadata *blockMeta = reinterpret_cast<BlockMetadata *>(
                oldSegment->mMetadataBufferForRecovery[i] +
                blocks[i] * Configuration::GetMetadataSize());

            if (i == Configuration::CalculateDiskId(mHeaderRegionSize + index,
                                                    mZones.size() - 1,
                                                    mZones.size())) {
                // parity
                mCodedBlockMetadata[index].lba = ~0ull;
            ...
            if (spdk_nvme_ns_cmd_write_with_md(
                    mZones[i]->GetDevice()->GetNamespace(),
                    mZones[i]->GetDevice()->GetIoQueue(0),
                    oldSegment->mDataBufferForRecovery[i] +
                        blocks[i] * Configuration::GetBlockSize(),
                    oldSegment->mMetadataBufferForRecovery[i] +
                        blocks[i] * Configuration::GetMetadataSize(),
                    mZones[i]->GetSlba() + mHeaderRegionSize + checkpoint +
                        maxStripeId,
                    1, completeOneEvent, &counter, 0, 0, 0) != 0) {
                fprintf(stderr, "Write error in rewriting the last group in "
                                "the new segment.\n");
            }
            // printf("%u %u %s\n", mCodedBlockMetadata[index].lba,
            // mCompactStripeTable[index], oldSegment->mDataBufferForRecovery[j]
            // + i * Configuration::GetBlockSize());
        }
        while (counter != 0) {
            for (uint32_t i = 0; i < mZones.size(); ++i) {
...
void Segment::SetZonesAndWpForRecovery(
...
uint32_t Segment::GetPos() { return mPos; }

void Segment::Dump()
{
    printf("Segment ID: %lu, n: %u, k: %u, numZones: %u, raidScheme: %u, "
           "position: %u, stripe offset: %u\n",
           mSegmentMeta.segmentId, mSegmentMeta.n, mSegmentMeta.k,
           mSegmentMeta.numZones, mPos, mPosInStripe);
    printf("\tZones: ");
