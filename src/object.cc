                              ...
                              ZstoreObject &obj)
{
    if (buffer_size < sizeof(LogEntry)) {
        if (Conf::IsDebugging())
            log_error("Buffer is too small to contain LogEntry, {}",
                      buffer_size);
    ...
    if (offset + sizeof(uint16_t) > buffer_size) {
        if (Conf::IsDebugging())
            log_error("Buffer overflow while reading key size");
    ...
    if (offset + sizeof(uint64_t) > buffer_size) {
        if (Conf::IsDebugging())
            log_error("Buffer overflow while reading data length");
        return false; // Buffer overflow
    }
    ...
    if (obj.datalen > 0) {
        ...
        if (obj.body == nullptr) {
            if (Conf::IsDebugging())
                log_error("Memory allocation failure while reading body");
...
{
    assert(bufferSize == Conf::GetChunkSize());
...
ChunkList deserializeDummyMap(std::string data, u64 num_chunks)
{
    ...
    for (u64 i = 0; i < num_chunks; i++) {
        chunk_list[i] = std::make_tuple(i, Conf::GetChunkSize());
    }
...
{
    assert(obj.datalen == Conf::GetObjectSizeInBytes());
    u64 num_chunks = obj.datalen / Conf::GetChunkSize();
    ...
    for (u64 i = 0; i < num_chunks; i++) {
        ...
        if (remaining_data_len >= Conf::GetChunkSize()) {
            chunk.datalen = Conf::GetChunkSize();
            remaining_data_len -= Conf::GetChunkSize();
        } else {
...
void mergeChunksIntoObject(std::vector<RequestContext *> chunk_vec,
                           std::string req_body)
{
    // auto chunk_len = Conf::GetChunkSize();
    //
