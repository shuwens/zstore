...
void spdk_nvme_zone_read_wrapper(
    struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair, void *data,
...
auto spdk_nvme_zone_read_async(struct spdk_nvme_ns *ns,
                               struct spdk_nvme_qpair *qpair, void *data,
                               uint64_t offset, uint32_t size, uint32_t flags)
    -> net::awaitable<const spdk_nvme_cpl *>
{
    auto init = [](auto completion_handler, spdk_nvme_ns *ns,
                   spdk_nvme_qpair *qpair, void *data, uint64_t offset,
                   uint32_t size, uint32_t flags) {
        spdk_nvme_zone_read_wrapper(ns, qpair, data, offset, size, flags,
                                    std::move(completion_handler));
    };

    return net::async_initiate<decltype(net::use_awaitable),
                               void(const spdk_nvme_cpl *)>(
        net::use_awaitable, init, ns, qpair, data, offset, size, flags);
}
...
auto zoneRead(void *arg1) -> net::awaitable<void>
{
    ...
    auto cpl = co_await spdk_nvme_zone_read_async(
        ioCtx.ns, ioCtx.qpair, ioCtx.data, ioCtx.offset, ioCtx.size, ioCtx.cb,
        ioCtx.ctx, ioCtx.flags, net::use_awaitable);
