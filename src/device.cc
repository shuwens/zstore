...
void Device::Init(struct spdk_nvme_ctrlr *ctrlr, int nsid, u32 zone_id)
{
    ...
    if (Conf::IsDebugging()) {
        if (spdk_nvme_ns_get_md_size(mNamespace) == 0) {
            log_info("ns: {}, Zone size: {}, zone cap: {}, num of zones: {}, "
                     "md size 0",
                     nsid, mZoneSize, mZoneCapacity, mNumZones);
            Conf::SetDeviceSupportMetadata(false);
        } else {
            log_info(
    ...
    opts.io_queue_size = Conf::GetQpairSize();

    mIoQueues = new struct spdk_nvme_qpair *[Conf::GetNumIoThreads()];
...
void Device::FinishZone(Zone *zone, void *ctx)
{
    ...
    //
    // if (Conf::GetBypassDevice()) {
    //     debug_e("Bypass finish zone");
    //     slot->Queue();
    //     return;
    // }
    //
    // if (Conf::GetEventFrameworkEnabled()) {
...
void Device::ReadZoneHeaders(std::map<uint64_t, uint8_t *> &zones)
{
    ...
    for (uint32_t i = 0; i < report->nr_zones; ++i) {
        ...
        uint8_t *buffer =
            (uint8_t *)spdk_zmalloc(Conf::GetBlockSize(), 4096, NULL,
