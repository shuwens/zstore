...
{
    uint32_t *ns_count_ref = (uint32_t *)cb_args;
...
static int cmd_enumerate(struct xnvme_cli *cli)
{
...
static int cmd_info(struct xnvme_cli *cli)
{
    ...
    xnvme_opts_pr(&o, XNVME_PR_DEF);

    return 0;
}

static inline int cmd_idfy_ctrlr(struct xnvme_cli *cli)
{
    struct xnvme_dev *dev = cli->args.dev;
    ...
    idfy = (struct xnvme_spec_znd_idfy *)xnvme_buf_alloc(dev, sizeof(*idfy));
    if (!idfy) {
        err = -errno;
    ...
    if (cli->args.data_output) {
        ...
        err = xnvme_buf_to_file((char *)idfy, sizeof(*idfy),
...
static inline int cmd_idfy_ns(struct xnvme_cli *cli)
{
    struct xnvme_dev *dev = cli->args.dev;
    struct xnvme_cmd_ctx ctx = xnvme_cmd_ctx_from_dev(dev);
    uint32_t nsid = xnvme_dev_get_nsid(cli->args.dev);
    ...
    xnvme_cli_pinf("xnvme_adm_idfy_ns: {nsid: 0x%x, csi: %s}", nsid,
                   xnvme_spec_csi_str(csi));

    idfy = (xnvme_spec_znd_idfy *)xnvme_buf_alloc(dev, sizeof(*idfy));
    if (!idfy) {
        err = -errno;
        xnvme_cli_perr("xnvme_buf_alloc()", err);
        goto exit;
...
static int cmd_report(struct xnvme_cli *cli)
{
    struct xnvme_dev *dev = cli->args.dev;
    ...
    xnvme_cli_pinf("Zone Information Report for lba: 0x%016lx, limit: %zu",
                   zslba, limit ? limit : geo->nzone);
...
static int cmd_changes(struct xnvme_cli *cli)
{
    const struct xnvme_spec_idfy_ctrlr *idfy_ctrlr =
        xnvme_dev_get_ctrlr_css(cli->args.dev);
...
static int cmd_errors(struct xnvme_cli *cli)
{
    struct xnvme_dev *dev = cli->args.dev;
    struct xnvme_cmd_ctx ctx = xnvme_cmd_ctx_from_dev(dev);
    ...
    log =
        (xnvme_spec_log_erri_entry *)xnvme_buf_alloc(cli->args.dev, log_nbytes);
    if (!log) {
        err = -errno;
        xnvme_cli_perr("xnvme_buf_alloc()", err);
        goto exit;
    ...
    err = xnvme_adm_log(&ctx, XNVME_SPEC_LOG_ERRI, nsid, 0, 0, 0, log,
                        log_nbytes);
    if (err || xnvme_cmd_ctx_cpl_status(&ctx)) {
...
static int cmd_read(struct xnvme_cli *cli)
{
    struct xnvme_dev *dev = cli->args.dev;
    struct xnvme_cmd_ctx ctx = xnvme_cmd_ctx_from_dev(dev);
    const struct xnvme_geo *geo = xnvme_dev_get_geo(dev);
    ...
    xnvme_cli_pinf("Reading nsid: 0x%x, slba: 0x%016lx, nlb: %zu", nsid, slba,
                   nlb);
    ...
    log_debug("Reading lba 0x{:x} blocks {}", slba, nlb);
...
static int cmd_write(struct xnvme_cli *cli)
{
    struct xnvme_dev *dev = cli->args.dev;
    struct xnvme_cmd_ctx ctx = xnvme_cmd_ctx_from_dev(dev);
    const struct xnvme_geo *geo = xnvme_dev_get_geo(dev);
    ...
    xnvme_cli_pinf("Writing nsid: 0x%x, slba: 0x%016lx, nlb: %zu", nsid, slba,
                   nlb);
...
static int cmd_append(struct xnvme_cli *cli)
{
    struct xnvme_dev *dev = cli->args.dev;
    struct xnvme_cmd_ctx ctx = xnvme_cmd_ctx_from_dev(dev);
    const struct xnvme_geo *geo = xnvme_dev_get_geo(dev);
    ...
    dbuf = (char *)xnvme_buf_alloc(dev, dbuf_nbytes);
    if (!dbuf) {
        err = -errno;
        xnvme_cli_perr("xnvme_buf_alloc()", err);
        goto exit;
...
static int _cmd_mgmt(struct xnvme_cli *cli, uint8_t action)
{
    struct xnvme_dev *dev = cli->args.dev;
    struct xnvme_cmd_ctx ctx = xnvme_cmd_ctx_from_dev(dev);
    uint32_t nsid = cli->args.nsid;
    ...
    xnvme_cli_pinf("MGMT: zslba: 0x%016lx, action: 0x%x, str: %s", zslba,
                   action,
                   xnvme_spec_znd_cmd_mgmt_send_action_str(
                       (xnvme_spec_znd_cmd_mgmt_send_action)action));

    if ((action == XNVME_SPEC_ZND_CMD_MGMT_SEND_DESCRIPTOR) &&
        cli->args.data_input) {
        xnvme_cli_pinf("Allocating dbuf");
        dbuf = (char *)xnvme_buf_alloc(dev, dbuf_nbytes);
        if (!dbuf) {
        ...
        err = xnvme_buf_fill(dbuf, dbuf_nbytes,
                             cli->args.data_input ? cli->args.data_input
                                                  : "anum");
        if (err) {
            xnvme_cli_perr("xnvme_buf_fill()", err);
            goto exit;
    ...
    err = xnvme_znd_mgmt_send(&ctx, nsid, zslba, select_all,
                              (xnvme_spec_znd_cmd_mgmt_send_action)action,
                              (xnvme_spec_znd_mgmt_send_action_so)0x0, dbuf);
    if (err || xnvme_cmd_ctx_cpl_status(&ctx)) {
...
static int cmd_mgmt(struct xnvme_cli *cli)
{
    return _cmd_mgmt(cli, cli->args.action);
}

static int cmd_mgmt_open(struct xnvme_cli *cli)
{
    return _cmd_mgmt(cli, XNVME_SPEC_ZND_CMD_MGMT_SEND_OPEN);
}

static int cmd_mgmt_finish(struct xnvme_cli *cli)
{
    return _cmd_mgmt(cli, XNVME_SPEC_ZND_CMD_MGMT_SEND_FINISH);
}

static int cmd_mgmt_close(struct xnvme_cli *cli)
{
    return _cmd_mgmt(cli, XNVME_SPEC_ZND_CMD_MGMT_SEND_CLOSE);
}

static int cmd_mgmt_reset(struct xnvme_cli *cli)
{
...
static struct xnvme_cli g_cli = {
    ...
    .descr_short = "Enumerate Zoned Namespaces, manage, inspect properties, "
                   "state, and send "
                   "IO to them",
