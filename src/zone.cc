...
{
    return (mSlba << 12) + offset * Conf::GetBlockSize();
}
...
void Zone::Write(uint32_t offset, uint32_t size, void *ctx)
{
    ...
    // }
    // mOffset += size / Conf::GetBlockSize();
}
...
uint32_t Zone::GetDeviceId() { return mDevice->GetDeviceId(); }

uint32_t Zone::GetIssuedPos() { return mOffset; }
...
uint32_t Zone::GetPos() { return mPos; }

bool Zone::IsFull() { return (mPos >= mCapacity); }

bool Zone::NoOngoingWrites() { return mOffset == mPos; }
...
uint32_t Zone::GetSlba() { return mSlba; }

uint32_t Zone::GetLength() { return mCapacity; }

uint32_t Zone::GetSize() { return mSize; }
...
void Zone::Release() { mDevice->AddAvailableZone(this); }
