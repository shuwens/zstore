...
void build_context(struct ibv_context *verbs, ZstoreController &ctrl);
void build_qp_attr(struct ibv_qp_init_attr *qp_attr);
void event_loop(struct rdma_event_channel *ec, int exit_on_disconnect,
                ZstoreController &ctrl);
void *poll_cq(void *);

void build_connection(struct rdma_cm_id *id, ZstoreController &ctrl)
{
    struct ibv_qp_init_attr qp_attr;

    build_context(id->verbs, ctrl);
                ...
                ZstoreController &ctrl)
{
    ...
    while (rdma_get_cm_event(ec, &event) == 0) {
        ...
        if (event_copy.event == RDMA_CM_EVENT_ADDR_RESOLVED) {
            build_connection(event_copy.id, ctrl);

            if (s_on_pre_conn_cb)
                s_on_pre_conn_cb(ctrl, event_copy.id);
                    ...
                    ZstoreController &ctrl)
{
    ...
    // conn->context.ctrl = ctrl;

    build_params(&cm_params);

    event_loop(ec, 1, ctrl); // exit on disconnect

    rdma_destroy_event_channel(ec);
}

void rc_server_loop(const char *port, ZstoreController &ctrl)
{
    ...
    event_loop(ec, 0, ctrl); // don't exit on disconnect
