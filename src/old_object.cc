...
fdb_status docio_init(struct docio_handle *handle, struct filemgr *file,
                      bool compress_document_body)
{
    ...
    if (file->config->kvssd) {
        malloc_align(handle->readbuffer, FDB_SECTOR_SIZE,
                     file->config->kvssd_retrieve_length +
...
void docio_free(struct docio_handle *handle) { free_align(handle->readbuffer); }
                                   ...
                                   size_t pos)
{
    ...
    if (pos + len_size <= blocksize) {
        ...
        return filemgr_write_offset(handle->file, bid, pos, len_size, zerobuf,
                                    false, handle->log_callback);
    } else {
...
bid_t docio_append_doc_raw(struct docio_handle *handle, uint64_t vernum,
                           uint64_t size, void *buf)
{
    ...
    // TODO: this ignores doc block marker writing
    if (handle->file->config->kvssd) {
        fs = filemgr_write_offset(handle->file, vernum, 0, size, buf, false,
                                  log_callback);
        return (bid_t)vernum;
    }
    ...
    if (!filemgr_is_writable(handle->file, handle->curblock)) {
        ...
        if (handle->curpos < real_blocksize) {
            ...
            filemgr_mark_stale(handle->file,
                               real_blocksize * handle->curblock +
                                   handle->curpos,
    ...
    if (size <= remaining_space) {
        ...
        if (non_consecutive) {
            ...
            // write meta
            fs = filemgr_write_offset(handle->file, handle->curblock, blocksize,
                                      sizeof(blk_meta), &blk_meta, false,
                                      log_callback);
        } else {
        ...
        if (fs != FDB_RESULT_SUCCESS) {
            fdb_log(
                log_callback, FDB_LOG_ERROR, fs,
                "Error in appending a doc block marker for a block id %" _F64
                " into a database file '%s'",
                handle->curblock, handle->file->filename);
        ...
        if (fs != FDB_RESULT_SUCCESS) {
            fdb_log(log_callback, FDB_LOG_ERROR, fs,
                    ...
                    ", offset %d, size %" _F64 " to a database file '%s'",
                    handle->curblock, offset, size, handle->file->filename);
            return BLK_NOT_FOUND;
        }
        handle->curpos += size;
        ...
        if (remain <= blocksize - handle->curpos &&
            ...
            if (begin != (handle->curblock + 1)) {
                fdb_log(
                    ...
                    handle->curblock + 1, handle->file->filename);
                return BLK_NOT_FOUND;
            }
            ...
            if (fs != FDB_RESULT_SUCCESS) {
                fdb_log(log_callback, FDB_LOG_ERROR, fs,
                        "Error in appending a doc block marker for a block id
                        "
                        "%" _F64 " into a database file '%s'",
                        handle->curblock, handle->file->filename);
                return BLK_NOT_FOUND;
            }
            if (offset > 0) {
            ...
            // next block to be allocated is not continuous .. allocate new
            // multiple blocks
            filemgr_alloc_multiple(handle->file,
                                   nblock + ((remain > 0) ? 1 : 0), &begin,
                                   &end, log_callback);
            offset = 0;
...
#else
        // Simple append mode
        // The given doc is appended at the byte offset right next the last
        doc.
        ...
        if (non_consecutive) {
            ...
            if (remain > offset) {
                // if the current block cannot accommodate the remaining
                length
            ...
            if (offset > 0 && !start_from_new_block) {
                ...
                // write meta
                fs = filemgr_write_offset(handle->file, handle->curblock,
                                          blocksize, sizeof(blk_meta),
                                          &blk_meta, false, log_callback);
                if (fs != FDB_RESULT_SUCCESS) {
                    fdb_log(log_callback, FDB_LOG_ERROR, fs,
                            "Error in appending a doc block metadata for a "
                            "block id %" _F64 " into a database file '%s'",
                            handle->curblock, handle->file->filename);
                    return BLK_NOT_FOUND;
                }
            ...
            if (filemgr_alloc_multiple_cond(
                    handle->file, handle->curblock + 1,
                    nblock + ((remain > offset) ? 1 : 0), &begin, &end,
                    log_callback) == handle->curblock + 1) {
                // start from current block
                if (begin != (handle->curblock + 1)) {
                    fdb_log(log_callback, FDB_LOG_ERROR, fs,
                            "Error in allocating blocks starting from block
                            id "
                ...
                if (fs != FDB_RESULT_SUCCESS) {
                    fdb_log(log_callback, FDB_LOG_ERROR, fs,
                            "Error in appending a doc block marker for a
                                block " " id %
                                " _F64 " into a database file '%s' ",
                                handle->curblock,
                            handle->file->filename);
        ...
        for (i = 0; i < block_list_size; ++i) {
            ...
            blk_meta.sb_bmp_revnum_hash =
                _endian_encode(handle->cur_bmp_revnum_hash);
            ...
            if (non_consecutive) {
                ...
                fs = _add_blk_marker(handle->file, block_list[i], blocksize,
                                     marker, log_callback);
            }
            if (fs != FDB_RESULT_SUCCESS) {
                fdb_log(log_callback, FDB_LOG_ERROR, fs,
                        "Error in appending a doc block marker for a block "
                        "id %" _F64 " into a database file '%s'",
                        block_list[i], handle->file->filename);
                return BLK_NOT_FOUND;
            }
            ...
            if (remainsize >= blocksize) {
                ...
                fs = filemgr_write_offset(handle->file, block_list[i], 0,
                                          blocksize, (uint8_t *)buf + offset,
                                          true, // mark block as immutable
                                          log_callback);
                if (fs != FDB_RESULT_SUCCESS) {
                    fdb_log(
                        ...
                        ", size %" _F64 " to a database file '%s'",
                        block_list[i], blocksize, handle->file->filename);
                    return BLK_NOT_FOUND;
                }
                offset += blocksize;
                ...
                if (fs != FDB_RESULT_SUCCESS) {
                    fdb_log(log_callback, FDB_LOG_ERROR, fs,
                            "Error in writing a doc block with id %" _F64 ",
                            " " size %
                                " _F64 " to a database file '%s' ",
                                block_list[i],
                            remainsize, handle->file->filename);
                    return BLK_NOT_FOUND;
                }
                offset += remainsize;
...
{
    return uint8_t(
        get_checksum(reinterpret_cast<const uint8_t *>(&length),
                     sizeof(keylen_t) + sizeof(uint16_t) + sizeof(uint32_t) * 2,
                     handle->file->crc_mode) &
        0xff);
}

INLINE bid_t _docio_append_doc(struct docio_handle *handle,
                               struct docio_object *doc)
{
    ...
    if (doc->length.bodylen > 0 && handle->compress_document_body) {
        ...
        ret = snappy_compress((char *)doc->body, length.bodylen,
                              (char *)compbuf, &_len);
    ...
    crc = get_checksum(reinterpret_cast<const uint8_t *>(buf),
                       docsize - 1 - sizeof(crc), handle->file->crc_mode);
    memcpy((uint8_t *)buf + offset, &crc, sizeof(crc));
    offset += sizeof(crc);
#endif

    memset((uint8_t *)buf + docsize - 1, 0xEF, 1);

    ret_offset = docio_append_doc_raw(handle, doc->vernum, docsize, buf);
...
{
    // Note: should adapt DOCIO_COMMIT_MARK_SIZE if this function is
    modified.uint32_t offset = 0;
    uint64_t docsize;
    ...
    vernum = filemgr_assign_new_vernum(handle->file);
    atomic_incr_uint64_t(&handle->file->pos);

    ret_offset = docio_append_doc_raw(handle, vernum, docsize, buf);
    free(buf);
...
bid_t docio_append_doc_system(struct docio_handle *handle,
                              struct docio_object *doc)
{
...
INLINE bool _docio_check_buffer(struct docio_handle *handle,
                                uint64_t bmp_revnum)
{
    size_t blocksize = handle->file->blocksize;
    bool non_consecutive = ver_non_consecutive_doc(handle->file->version);
    ...
    if (non_consecutive) {
        // new version: support non-consecutive document block
        blocksize -= DOCBLK_META_SIZE;
        memcpy(&blk_meta, (uint8_t *)handle->readbuffer + blocksize,
               sizeof(blk_meta));
    } else {
        // old version: block marker only
        blocksize -= BLK_MARKER_SIZE;
...
INLINE bool _docio_check_buffer_kvssd(struct docio_handle *handle,
                                      uint64_t bmp_revnum)
{
    uint8_t marker;

    if (handle->last_read_size > 0) {
        marker = *((uint8_t *)(handle->readbuffer) + handle->last_read_size -
                   BLK_MARKER_SIZE);
    } else {
        marker = *((uint8_t *)(handle->readbuffer) + handle->file->blocksize -
                   BLK_MARKER_SIZE);
    }

    if (marker == BLK_MARKER_BNODE || marker == BLK_MARKER_DBHEADER) {
        memset(handle->readbuffer, 0x0, handle->file->blocksize);
        return false;
    } else {
        return true;
    }
...
INLINE bool _docio_check_buffer_kvssd(struct docio_handle *handle,
                                      uint64_t bmp_revnum, void *buf,
                                      uint32_t len)
{
    uint8_t marker;

    marker = *((uint8_t *)(buf) + len - BLK_MARKER_SIZE);
...
static int64_t _docio_read_length(struct docio_handle *handle, uint64_t offset,
                                  ...
                                  bool read_on_cache_miss)
{
    ...
    if (handle->file->config->kvssd) {
        ...
        fs = _docio_read_through_buffer(handle, offset, log_callback,
                                        read_on_cache_miss);
    } else {
    ...
    if (fs != FDB_RESULT_SUCCESS) {
        if (read_on_cache_miss) {
            fdb_log(log_callback, FDB_LOG_ERROR, fs,
                    ...
                    " in block id %" _F64 " from a database file '%s'",
                    offset, bid, handle->file->filename);
        }
        return (int64_t)fs;
    }

    if (!handle->file->config->kvssd) {
        // TODO: might be necessary to have some sort of flag
        // for cached KV pairs as we don't have meta markers
        // at the end of blocks since we don't use blocks
        if (!_docio_check_buffer(handle, (uint64_t)-1)) {
            return (int64_t)
                FDB_RESULT_READ_FAIL; // Need to define a better error code
        }
    } else {
        pos = 0;
    }
    ...
    if (handle->file->config->kvssd ||
        restsize >= sizeof(struct docio_length)) {
        ...
        if (non_consecutive) {
            ...
            if (bid == BLK_NOT_FOUND) {
                // Reached the last block. Simply return the offset that is
                                         ...
                                         err_log_callback *log_callback)
{
    ...
    while (rest_len > 0) {
        ...
        if (fs != FDB_RESULT_SUCCESS) {
            fdb_log(log_callback, FDB_LOG_ERROR, fs,
                    "Error in reading a doc block with block id %" _F64 "
                    from " " a database file '%s' ", bid,
                    handle->file->filename);
            return (int64_t)fs;
        }
        restsize = blocksize - pos;
        ...
        if (restsize >= rest_len) {
            memcpy((uint8_t *)buf_out + (len - rest_len), (uint8_t *)buf + pos,
                   rest_len);
            pos += rest_len;
            rest_len = 0;
        } else {
            ...
            if (rest_len > 0 && bid >= filemgr_get_pos(handle->file) /
                                           handle->file->blocksize) {
                // no more data in the file .. the file is corrupted
                fdb_log(log_callback, FDB_LOG_FATAL, FDB_RESULT_FILE_CORRUPTION,
...
static int64_t _docio_read_doc_component_kvssd(struct docio_handle *handle,
                                               uint64_t offset, uint64_t pos,
                                               uint32_t len, void *buf_out,
                                               err_log_callback *log_callback)
{
    uint32_t rest_len;
    size_t blocksize = handle->file->config->kvssd_max_value_size;
    bool non_consecutive = ver_non_consecutive_doc(handle->file->version);
    struct docblk_meta blk_meta;
#ifdef __CRC32
    if (non_consecutive) {
...
static int64_t _docio_read_doc_component_comp(struct docio_handle *handle,
                                              uint64_t offset, uint32_t len,
                                              uint32_t comp_len, void *buf_out,
                                              void *comp_data_out,
                                              err_log_callback *log_callback)
{
    int ret;
    ...
    _offset = _docio_read_doc_component(handle, offset, comp_len, comp_data_out,
                                        log_callback);
    if (_offset < 0) {
    ...
    uncomp_size = len;
    ret = snappy_uncompress((char *)comp_data_out, comp_len, (char *)buf_out,
                            &uncomp_size);
    if (ret < 0) {
        fdb_log(log_callback, FDB_LOG_ERROR, FDB_RESULT_COMPRESSION_FAIL,
                ...
                " " offset %
                    " _F64 ",
                length % d from a database file '%s' ",
                    offset,
                len, handle->file->filename);
    ...
    if (uncomp_size != len) {
        fdb_log(log_callback, FDB_LOG_ERROR, FDB_RESULT_COMPRESSION_FAIL,
                ...
                "%" _F64
                " in a database file '%s', because the uncompressed length %d
                " " is not same as the expected length %
                    d ", offset,
                    handle->file->filename,
                uncomp_size, len);
        return (int64_t)FDB_RESULT_COMPRESSION_FAIL;
...
static int64_t _docio_read_doc_component_comp_kvssd(
    struct docio_handle *handle, uint64_t offset, uint64_t pos, uint32_t len,
    uint32_t comp_len, void *buf_out, void *comp_data_out,
    err_log_callback *log_callback)
{
    int ret;
    size_t uncomp_size;
    ...
    _offset = _docio_read_doc_component_kvssd(handle, offset, pos, comp_len,
                                              comp_data_out, log_callback);
    if (_offset < 0) {
        fdb_log(log_callback, FDB_LOG_ERROR, (fdb_status)_offset,
                "Error in reading the file with offset %" _F64 ", length %d "
                "from a database file '%s'",
...
fdb_status docio_read_doc_length(struct docio_handle *handle,
                                 struct docio_length *length, uint64_t offset)
{
    uint8_t checksum;
    int64_t _offset;
    ...
    if (checksum != _length.checksum) {
        fdb_log(log_callback, FDB_LOG_ERROR, FDB_RESULT_CHECKSUM_ERROR,
                ...
                handle->file->filename, checksum, _length.checksum,
                _length.keylen, _length.metalen, _length.bodylen,
                _length.bodylen_ondisk, offset);
    ...
    *length = _docio_length_decode(_length);
    if (length->keylen == 0 || length->keylen > FDB_MAX_KEYLEN_INTERNAL) {
        fdb_log(log_callback, FDB_LOG_ERROR, FDB_RESULT_FILE_CORRUPTION,
                ...
                "bodylen_ondisk %d offset %" _F64,
                handle->file->filename, checksum, _length.keylen,
                              ...
                              keylen_t *keylen, void *keybuf)
{
    ...
    if (checksum != _length.checksum) {
        fdb_log(log_callback, FDB_LOG_ERROR, FDB_RESULT_CHECKSUM_ERROR,
                "doc_length key checksum mismatch error in a database file
                        '%s' " " crc %
                        x !=
                    % x(crc in doc) keylen % d metalen % d bodylen %
                        d " " bodylen_ondisk % d offset %
                        " _F64,
                        handle->file->filename,
    ...
    if (handle->file->config->kvssd) {
        _offset = _docio_read_doc_component_kvssd(
            handle, offset, _offset, length.keylen, keybuf, log_callback);
    } else {
    ...
    if (_offset < 0) {
        fdb_log(log_callback, FDB_LOG_ERROR, (fdb_status)_offset,
                ...
                "from a database file '%s'",
                _offset, length.keylen, handle->file->filename);
        return (fdb_status)_offset;
    }
                                ...
                                bool read_on_cache_miss)
{
    ...
    if (doc->length.keylen == 0 ||
        ...
        fdb_log(log_callback, FDB_LOG_ERROR, FDB_RESULT_FILE_CORRUPTION,
                ...
                " " a database file '%s' ", doc->length.keylen,
                handle->file->filename);
        return (int64_t)FDB_RESULT_FILE_CORRUPTION;
    }
...
int64_t docio_read_doc(struct docio_handle *handle, uint64_t offset,
                       struct docio_object *doc, bool read_on_cache_miss)
{
    uint8_t checksum;
    int64_t _offset = 0;
    int key_alloc = 0;
                         ...
                         struct docio_object *doc, void *buf)
{
    ...
    _length = *(struct docio_length *)buf;
    _offset += sizeof(struct docio_length);
    ...
    if (doc->length.flag & DOCIO_TXN_COMMITTED) {
        ...
        uint64_t doc_offset;
        memcpy(&doc_offset, (uint8_t *)buf + _offset, sizeof(doc_offset));
        _offset += doc->length.keylen;
    ...
    memcpy(doc->key, (uint8_t *)buf + _offset, doc->length.keylen);
    _offset += doc->length.keylen;

    memcpy(&_timestamp, (uint8_t *)buf + _offset, sizeof(timestamp_t));
    _offset += sizeof(timestamp_t);
    doc->timestamp = _endian_decode(_timestamp);

    memcpy((void *)&_seqnum, (uint8_t *)buf + _offset, sizeof(fdb_seqnum_t));
    _offset += sizeof(fdb_seqnum_t);
    doc->seqnum = _endian_decode(_seqnum);

    memcpy(doc->meta, (uint8_t *)buf + _offset, doc->length.metalen);
    _offset += doc->length.metalen;

#ifdef _DOC_COMP
    fdb_log(log_callback, FDB_LOG_ERROR, FDB_RESULT_READ_FAIL,
            "Asserting 0 in docio_buf_to_doc");
    assert(0);
    if (doc->length.flag & DOCIO_COMPRESSED) {
        comp_body = (void *)malloc(doc->length.bodylen_ondisk);
        if (handle->file->config->kvssd) {
...
#else
    memcpy(doc->body, (uint8_t *)buf + _offset, doc->length.bodylen);
    _offset += doc->length.bodylen;
#endif
                                     ...
                                     bool keymeta_only)
{
    ...
    if (num_sub < 0) {
        ...
        fdb_log(handle->log_callback, FDB_LOG_ERROR, (fdb_status)num_sub,
                "Error in submitting async I/O requests to a file '%s', errno
                " "msg: %s", handle->file->filename, errno_msg);
        return num_sub;
        ...
        handle->file->ops->get_errno_str(errno_msg, 512);
        fdb_log(
            handle->log_callback, FDB_LOG_ERROR, (fdb_status)num_sub,
            "Error in submitting async I/O requests to a file '%s', errno
            msg: "
            "%s, "
            "%d requests were submitted, but only %d requests were
            processed", handle->file->filename, errno_msg, size, num_sub);
        return num_sub;
    }

    while (num_sub > 0) {
        num_events = handle->file->ops->aio_getevents(aio_handle, 1, num_sub,
                                                      (unsigned int)-1);
        if (num_events < 0) {
            // Error loggings
            char errno_msg[512];
            handle->file->ops->get_errno_str(errno_msg, 512);
            fdb_log(
                handle->log_callback, FDB_LOG_ERROR, (fdb_status)num_events,
                "Error in getting async I/O events from the completion queue
                " "for a file '%s', errno msg: %s", handle->file->filename,
                errno_msg);
            return num_events;
        }
        num_sub -= num_events;
        for (io_evt = aio_handle->events; num_events > 0;
             --num_events, ++io_evt) {
            ...
            // read, then they will be sequentially read through the
            synchronous
                // I/O path (i.e., buffer cache -> disk read if cache miss).
                // As these additional blocks are sequential reads, we don't
                expect
            ...
            if (keymeta_only) {
                ...
                _offset =
                    docio_read_doc(handle, offset, &doc_array[doc_idx], true);
            }
            if (_offset <= 0) {
                ++doc_idx;
...
size_t
docio_batch_read_docs(struct docio_handle *handle, uint64_t *offset_array,
                      struct docio_object *doc_array, size_t array_size,
                      size_t data_size_threshold, size_t batch_size_threshold,
                      struct async_io_handle *aio_handle, bool keymeta_only)
{
    size_t i = 0;
    ...
    if (aio_handle) {
        // If async I/O is supported, we will then read non-resident docs
        from
            // disk by using async I/O operations.
            read_on_cache_miss = false;
    }

    for (i = 0; i < array_size && i < batch_size_threshold &&
                sum_doc_size < data_size_threshold;
         ++i) {
        memset(&doc_array[doc_idx], 0x0, sizeof(struct docio_object));
        if (keymeta_only) {
            _offset = docio_read_doc_key_meta(handle, offset_array[i],
                                              &doc_array[doc_idx],
                                              read_on_cache_miss);
        } else {
            _offset = docio_read_doc(handle, offset_array[i],
                                     &doc_array[doc_idx], read_on_cache_miss);
        }
        if (_offset <= 0) {
            if (aio_handle) {
                // The page is not resident in the cache. Prepare and perform
                // Async I/O
                handle->file->ops->aio_prep_read(aio_handle, aio_size,
                                                 block_size, offset_array[i]);
                if (++aio_size == (int)aio_handle->queue_depth) {
                    int num_sub = _submit_async_io_requests(
                        handle, doc_array, doc_idx, aio_handle, aio_size,
    ...
    if (aio_size && !read_fail) {
        int num_sub =
            _submit_async_io_requests(handle, doc_array, doc_idx, aio_handle,
                                      aio_size, &sum_doc_size, keymeta_only);
        if (num_sub < 0) {
            read_fail = true;
        } else {
            doc_idx += num_sub;
...
bool docio_check_buffer(struct docio_handle *handle, bid_t bid,
