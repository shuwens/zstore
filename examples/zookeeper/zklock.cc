...
*/
void zk_get_children_lock_res1(int rc, const struct String_vector *strings,
                               const void *data)
{
...
void zk_create_lock_res1_es(int rc, const char *value, const void *data)
{
    ...
    if (rc == 0) {
        ...
        if (seq > 0) {
            int ret;
            ret = zoo_aget_children(zk_hdl, "/lock/res1", 0,
                                    zk_get_children_lock_res1, data);
            if (ret) {
...
*/
void zk_watcher_disconnect(zhandle_t *zh, int type, int state, const char *path,
                           void *ctx)
{
    if (0 == strcmp(zmt.watch, path)) {
        int ret = zoo_aget_children(zh, "/lock/res1", 0,
                                    zk_get_children_lock_res1, zh);
        if (ret) {
...
int main(int argc, const char *argv[])
{
    ...
    /* 初始化 zk */
    zhandle_t *zk_hdl =
        zookeeper_init("127.0.0.1:2181", zk_watcher_disconnect, 30000, 0, 0, 0);
