...
{
    int ret = zoo_create(zk_handle, g_path.c_str(), g_data.c_str(),
                         g_data.size(), &ZOO_OPEN_ACL_UNSAFE, 0, NULL, 0);
    if (ZOK != ret) {
        fprintf(stderr, "%d: %s\n", ret, zerror(ret));
        return -1;
    }
...
{
    int ret =
        zoo_set(zk_handle, g_path.c_str(), g_data.c_str(), g_data.size(), -1);
    if (ZOK != ret) {
        fprintf(stderr, "%d: %s\n", ret, zerror(ret));
        return -1;
    }
...
int Execute()
{
    ...
    zhandle_t *zk_handle =
        zookeeper_init("localhost:2181", NULL, 30000, 0, NULL, 0);
    if (NULL == zk_handle) {
...
int PrintHelp()
{
    ...
    printf("example: ./zk -b 127.0.0.1:2181,127.0.0.2:2181,127.0.0.3:2181 -c "
           "/test_node \"helloworld\" \n");
