// https://github.com/talawahtech/seastar/blob/http-performance/apps/tcp_httpd/tcp_httpd.cc
/*
      ...
      public:
        ...
        future<> process()
        {
            ...
            return _read_buf.read().then([this](temporary_buffer<char> buf) {
                ...
                return _write_buf.write(str_json)
                    .then([this] { return _write_buf.flush(); })
                    .then([this] { return this->process(); });
            });
        }
    };
};
    ...
    return app.run_deprecated(ac, av, [&] {
        ...
        (void)server->start()
            .then([server = std::move(server), port]() mutable {
                ...
                // Use cpu_id->shard_id mapping to ensure that socket 0 is
                // started on cpu 0 etc for SO_ATTACH_REUSEPORT_CBPF to work
                // efficiently
                std::optional<std::vector<unsigned>> cpu_to_shard =
                    smp::get_cpu_to_shard_mapping();
                if (cpu_to_shard) {
                    auto all_cpus = smp::all_cpus();
                    return do_for_each(all_cpus, [server = std::move(server),
                                                  cpu_to_shard =
                                                      std::move(cpu_to_shard),
                                                  port](auto cpu_id) {
                        auto shard_id = cpu_to_shard.value()[cpu_id];
                        std::cout << "CPU: " << cpu_id << ", Socket: " << cpu_id
                                  << ", Shard: " << shard_id << "\n";
                        return server->invoke_on(shard_id, &tcp_server::listen,
                                                 ipv4_addr{port});
                    });
                } else {
                    return server->invoke_on_all(&tcp_server::listen,
                                                 ipv4_addr{port});
                }
            })
            .then([port] {
