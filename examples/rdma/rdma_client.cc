    ...
    while ((option = getopt(argc, argv, "s:a:p:")) != -1) {
        ...
        case 's':
            printf("Passed string is : %s , with count %u \n",
                   input_str.c_str(), (unsigned int)strlen(input_str.c_str()));

            src = static_cast<char *>(calloc(strlen(input_str.c_str()), 1));
            ...
            dst = static_cast<char *>(calloc(strlen(input_str.c_str()), 1));
