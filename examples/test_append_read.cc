...
// static TAILQ_HEAD(,
//                   worker_thread) g_workers =
//                   TAILQ_HEAD_INITIALIZER(g_workers);
...
// static void io_read_complete(void *ctx, const struct spdk_nvme_cpl
// *completion); static void get_arb_feature(struct spdk_nvme_ctrlr *ctrlr);
// static int set_arb_feature(struct spdk_nvme_ctrlr *ctrlr);
// static const char *print_qprio(enum spdk_nvme_qprio);
//
// static void register_ns(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_ns
// *ns)
// {
//     struct ns_entry *entry;
...
//         ctrlr, SPDK_NVME_INTEL_FEAT_LATENCY_TRACKING, latency_tracking.raw,
//         0, NULL, 0, enable_latency_tracking_complete, NULL);
//     if (res) {
...
//     opts.qprio = qprio;
//
//     ns_ctx->qpair = spdk_nvme_ctrlr_alloc_io_qpair(ctrlr, &opts,
//     sizeof(opts)); if (!ns_ctx->qpair) {
...
//     disabled]\n"); printf("\t\t(0 - disabled; 1 - enabled)\n"); printf("\t[-n
//     subjected IOs for performance comparison]\n"); printf("\t[-i shared
//     memory group ID]\n"); printf("\t[-r remote NVMe over Fabrics target
//     address]\n"); printf("\t[-g use single file descriptor for DPDK memory
//     segments]\n");
// }
//
...
//     "
//            "%d -i %d\n",
//            program_name, g_arbitration.queue_depth,
//            g_arbitration.io_size_bytes, g_arbitration.workload_type,
...
//         if (g_arbitration.arbitration_mechanism == SPDK_NVME_CAP_AMS_WRR) {
//             qprio =
//                 static_cast<enum spdk_nvme_qprio>(static_cast<int>(qprio) +
//                 1);
//         }
//
...
//         }
//
//         ns_ctx = (struct ns_worker_ctx *)malloc(sizeof(struct
//         ns_worker_ctx)); if (!ns_ctx) {
//             return 1;
//         }
...
//         arb.feat_arbitration.raw =
//         features[SPDK_NVME_FEAT_ARBITRATION].result;
//
...
//     if (!features[SPDK_NVME_FEAT_ARBITRATION].valid) {
//         printf(
//             "Set Arbitration Feature failed and use default
//             configuration\n");
//     }
//
//     return 0;
