...
TAILQ_HEAD(, nvmf_target_poll_group)
g_poll_groups = TAILQ_HEAD_INITIALIZER(g_poll_groups);
...
static void usage(char *program_name)
{
    ...
    printf("\n");
    printf("\t[-g period of round robin poll group migration (us) (default: 0 "
           "(disabled))]\n");
...
#define NVMF_NO_HUGE 257
    {"no-huge", no_argument, NULL, NVMF_NO_HUGE}, {0, 0, 0, 0}};

static int parse_args(int argc, char **argv, struct spdk_env_opts *opts)
{
    int op, opt_index;
    long int value;

    while ((op = getopt_long(argc, argv, NVMF_GETOPT_STRING,
                             g_nvmf_cmdline_opts, &opt_index)) != -1) {
        switch (op) {
        case 'g':
...
static int nvmf_reactor_run(void *arg)
{
    ...
    do {
        if (spdk_ring_dequeue(nvmf_reactor->threads, (void **)&lw_thread, 1)) {
            ...
            if (spdk_unlikely(spdk_thread_is_exited(thread) &&
                ...
                spdk_ring_enqueue(nvmf_reactor->threads, (void **)&lw_thread, 1,
                                  NULL);
            }
        }
    ...
    while (spdk_ring_dequeue(nvmf_reactor->threads, (void **)&lw_thread, 1)) {
        ...
        if (spdk_thread_is_exited(thread)) {
            ...
            /* This thread is not exited yet, and may need to communicate with
             * other threads to be exited. So mark it as exiting, and check
             * again after traversing other threads.
             */
...
static void nvmf_request_spdk_thread_reschedule(struct spdk_thread *thread)
{
    struct nvmf_lw_thread *lw_thread;
...
static int nvmf_init_threads(void)
{
    int rc;
    uint32_t i;
    char thread_name[32];
     ...
     * framework. The size of the extra memory allocated is the second
     * parameter.
     */
    spdk_thread_lib_init_ext(
        nvmf_reactor_thread_op, nvmf_reactor_thread_op_supported,
        sizeof(struct nvmf_lw_thread), SPDK_DEFAULT_MSG_MEMPOOL_SIZE);

    /* Spawn one system thread per CPU core. The system thread is called a
     * reactor. SPDK will spawn lightweight threads that must be mapped to
     * reactors in nvmf_schedule_spdk_thread. Using a single system thread per
     * CPU core is a choice unique to this application. SPDK itself does not
     * require this specific threading model. For example, another viable
     * threading model would be dynamically scheduling the lightweight threads
     * onto a thread pool using a work queue.
     */
    SPDK_ENV_FOREACH_CORE(i)
    {
        ...
        nvmf_reactor->threads = spdk_ring_create(SPDK_RING_TYPE_MP_SC, 1024,
        ...
        if (i == main_core) {
            ...
            rc = spdk_env_thread_launch_pinned(i, nvmf_reactor_run,
                                               nvmf_reactor);
            if (rc) {
    ...
    /* Spawn a lightweight thread only on the current core to manage this
     * application. */
...
static void nvmf_destroy_threads(void)
{
...
static void nvmf_tgt_destroy_done(void *ctx, int status)
{
...
static void nvmf_destroy_nvmf_tgt(void)
{
    if (g_nvmf_tgt.tgt) {
...
static void nvmf_create_nvmf_tgt(void)
{
     ...
     */
    subsystem =
        spdk_nvmf_subsystem_create(g_nvmf_tgt.tgt, SPDK_NVMF_DISCOVERY_NQN,
...
static void nvmf_tgt_subsystem_stop_next(struct spdk_nvmf_subsystem *subsystem,
                                         void *cb_arg, int status)
{
    ...
    if (subsystem) {
        rc = spdk_nvmf_subsystem_stop(subsystem, nvmf_tgt_subsystem_stop_next,
                                      cb_arg);
        if (rc) {
            nvmf_tgt_subsystem_stop_next(subsystem, cb_arg, 0);
            fprintf(stderr,
                    "Unable to stop NVMe-oF subsystem. Trying others.\n");
        }
        return;
    }
...
static void nvmf_tgt_stop_subsystems(struct nvmf_target *nvmf_tgt)
{
    struct spdk_nvmf_subsystem *subsystem;
    int rc;
...
static void nvmf_tgt_subsystem_start_next(struct spdk_nvmf_subsystem *subsystem,
                                          void *cb_arg, int status)
{
    int rc;
    ...
    if (subsystem) {
        ...
        if (rc) {
            ...
            fprintf(stderr,
                    "Unable to start NVMe-oF subsystem. shutting down app.\n");
            nvmf_target_advance_state();
        }
        return;
    }
...
static void nvmf_tgt_start_subsystems(struct nvmf_target *nvmf_tgt)
{
    struct spdk_nvmf_subsystem *subsystem;
    int rc;
    ...
    subsystem = spdk_nvmf_subsystem_get_first(nvmf_tgt->tgt);
    if (spdk_likely(subsystem)) {
        /* In SPDK there are three states in subsystem: Inactive, Active,
         * Paused. Start subsystem means make it from inactive to active that
         * means subsystem start to work or it can be accessed.
         */
        rc = spdk_nvmf_subsystem_start(subsystem, nvmf_tgt_subsystem_start_next,
                                       NULL);
        if (rc) {
...
static void nvmf_tgt_create_poll_groups_done(void *ctx)
{
...
/* Create a lightweight thread per poll group instead of assuming a pool of
 * lightweight threads already exist at start up time. A poll group is a
 * collection of unrelated NVMe-oF connections. Each poll group is only accessed
 * from the associated lightweight thread.
 */
static void nvmf_poll_groups_create(void)
{
    ...
    SPDK_ENV_FOREACH_CORE(i)
    {
        ...
        snprintf(thread_name, sizeof(thread_name), "nvmf_tgt_poll_group_%03u",
                 i);
...
static void _nvmf_tgt_destroy_poll_groups_done(void *ctx)
{
...
static void nvmf_tgt_destroy_poll_groups_done(void *cb_arg, int status)
{
    struct nvmf_target_poll_group *pg = cb_arg;

    free(pg);

    spdk_thread_send_msg(g_fini_thread, _nvmf_tgt_destroy_poll_groups_done,
                         NULL);

    spdk_thread_exit(spdk_get_thread());
}

static void nvmf_tgt_destroy_poll_group(void *ctx)
{
    struct nvmf_target_poll_group *pg = ctx;

    spdk_nvmf_poll_group_destroy(pg->group, nvmf_tgt_destroy_poll_groups_done,
                                 pg);
}

static void nvmf_poll_groups_destroy(void)
{
...
static void nvmf_subsystem_fini_done(void *cb_arg)
{
...
static void nvmf_subsystem_init_done(int rc, void *cb_arg)
{
...
static void migrate_poll_group_by_rr(void *ctx)
{
...
static int migrate_poll_groups_by_rr(void *ctx)
{
    struct nvmf_target_poll_group *pg;
    ...
    do {
        ...
        case NVMF_RUNNING:
            ...
            if (g_migrate_pg_period_us != 0) {
                g_migrate_pg_poller = SPDK_POLLER_REGISTER(
                    migrate_poll_groups_by_rr, NULL, g_migrate_pg_period_us);
            }
            break;
...
static void _nvmf_shutdown_cb(void *ctx)
{
...
static void nvmf_shutdown_cb(int signo)
{
    if (!g_intr_received) {
...
static int nvmf_setup_signal_handlers(void)
{
