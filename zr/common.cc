...
RequestContextPool::RequestContextPool(uint32_t cap)
{
    ...
    for (uint32_t i = 0; i < capacity; ++i) {
        ...
        contexts[i].metadataBuffer = (uint8_t *)spdk_zmalloc(
            Configuration::GetMetadataSize(), Configuration::GetMetadataSize(),
    ...
    if (availableContexts.empty() && force == false) {
        ...
        if (!availableContexts.empty()) {
            ...
            ctx->dataBuffer = (uint8_t *)spdk_zmalloc(
                Configuration::GetBlockSize(), 4096, NULL,
                SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
            ctx->metadataBuffer = (uint8_t *)spdk_zmalloc(
                Configuration::GetMetadataSize(),
                Configuration::GetMetadataSize(), NULL, SPDK_ENV_SOCKET_ID_ANY,
                SPDK_MALLOC_DMA);
            ctx->Clear();
            ctx->available = false;
...
ReadContextPool::ReadContextPool(uint32_t cap, RequestContextPool *rp)
{
    ...
    for (uint32_t i = 0; i < capacity; ++i) {
        ...
        contexts[i].metadata =
            (uint8_t *)spdk_zmalloc(Configuration::GetStripeSize(), 4096, NULL,
                                    SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
...
StripeWriteContextPool::StripeWriteContextPool(uint32_t cap,
                                               struct RequestContextPool *rp)
{
    capacity = cap;
    rPool = rp;
    ...
    for (int i = 0; i < capacity; ++i) {
        contexts[i].data = new uint8_t
            *[Configuration::GetStripeSize() / Configuration::GetBlockSize()];
        contexts[i].metadata = new uint8_t
            *[Configuration::GetStripeSize() / Configuration::GetBlockSize()];
        contexts[i].ioContext.clear();
        availableContexts.emplace_back(&contexts[i]);
    }
...
{
    for (auto it = inflightContexts.begin(); it != inflightContexts.end();) {
...
{
    // Level raidLevel = Configuration::GetRaidLevel();
    // if (raidLevel == 1) {
    ...
    uint8_t decodeMatrix[n * k]; // [k:k] to [n:k] stores coefficients for the
                                 // to-decode chunk part
    ...
    for (uint32_t i = 0; i < n; ++i) {
        uint32_t zid = Configuration::CalculateDiskId(offset, i, n);
        if (alive[zid]) {
            mapping[i] = zid;
        } else {
            mapping[i] = ~0u;
    ...
    for (uint32_t i = 0, j = 0; i < n; ++i) {
        if (mapping[i] == ~0u)
            continue; // alive[i] == false must implies that i is not
                      // decodeIndex
        if (j == k)
            break;
        memcpy(recoverMatrix + j * k, gEncodeMatrix + i * k,
               k * sizeof(uint8_t));
        j++;
    }
    ...
    if (decodeIndex < k) { // a data block need decoding
        memcpy(decodeMatrix + (0 + k) * k, invRecoverMatrix + decodeIndex * k,
               k * sizeof(uint8_t));
...
void EncodeStripe(uint8_t **stripe, uint32_t n, uint32_t k, uint32_t unitSize)
{
    ...
    // if (Configuration::GetRaidLevel() == 1) {
